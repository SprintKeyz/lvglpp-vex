...
protected:
  ...
  /** \fn virtual lv_fs_res_t read_cb(void * file_p, void * buf, uint32_t btr,
   * uint32_t * br) \brief Reads data from a file. \param file_p: pointer to the
   * file descriptor. \param buf: pointer to the recipient data buffer. \param
   * btr: number of bytes to read. \param br: number of bytes actually read.
   *  \returns result code: LV_FS_RES_OK if successful, LV_RES_* otherwise.
   */
  virtual lv_fs_res_t read_cb(void *file_p, void *buf, uint32_t btr,
                              uint32_t *br) = 0;

  /** \fn virtual lv_fs_res_t write_cb(void * file_p, const void * buf, uint32_t
   * btw, uint32_t * bw) \brief Writes data to a file. \param file_p: pointer to
   * the file descriptor. \param buf: pointer to the recipient data buffer.
   *  \param btw: number of bytes to write.
   *  \param bw: number of bytes actually written.
   *  \returns result code: LV_FS_RES_OK if successful, LV_RES_* otherwise.
   */
  virtual lv_fs_res_t write_cb(void *file_p, const void *buf, uint32_t btw,
                               uint32_t *bw) = 0;

  /** \fn virtual lv_fs_res_t seek_cb(void * file_p, uint32_t pos,
   * lv_fs_whence_t whence) \brief Moves access pointer within file. \param
   * file_p: pointer to the file descriptor. \param pos: position to move to.
   *  \param whence: mode; absolute (LV_FS_SEEK_SET), relative (LV_FS_SEEK_CUR),
   *  or absolute from end of file (LV_FS_SEEK_END).
   *  \returns result code: LV_FS_RES_OK if successful, LV_RES_* otherwise.
   */
  virtual lv_fs_res_t seek_cb(void *file_p, uint32_t pos,
                              lv_fs_whence_t whence) = 0;
...
class V5FileSystem : public FileSystem {
private:
  ...
  lv_fs_res_t read_cb(void *file_p, void *buf, uint32_t btr,
                      uint32_t *br) override;

  lv_fs_res_t write_cb(void *file_p, const void *buf, uint32_t btw,
                       uint32_t *bw) override;

  lv_fs_res_t seek_cb(void *file_p, uint32_t pos,
                      lv_fs_whence_t whence) override;
...
public:
  V5FileSystem(char letter) : FileSystem(letter) {}
};

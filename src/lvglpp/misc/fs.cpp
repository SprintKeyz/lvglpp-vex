...
FileSystem::FileSystem(char letter) {
  ...
  auto f_open = [](lv_cls_ptr drv, const char *path,
...
File::File() { this->lv_obj = LvPointerType(lv_cls_alloc<lv_cls>()); }
...
lv_fs_res_t File::close() { return lv_fs_close(this->raw_ptr()); }

std::string File::read(uint32_t btr) {
  std::string buf(btr, '\0');
  uint32_t nr;
  auto res =
      lv_fs_read(this->raw_ptr(), reinterpret_cast<void *>(&buf[0]), btr, &nr);
  if (res != LV_FS_RES_OK)
    return std::string{};
  return buf;
}
...
uint32_t File::write(const std::string &data) {
  uint32_t bw;
  lv_fs_write(this->raw_ptr(), reinterpret_cast<const void *>(data.c_str()),
              data.size(), &bw);
  return bw;
}
...
Directory::Directory() { this->lv_obj = LvPointerType(lv_cls_alloc<lv_cls>()); }
...
lv_fs_res_t Directory::close() { return lv_fs_dir_close(this->raw_ptr()); }
...
bool V5FileSystem::ready_cb() {
  LV_LOG_INFO("calling ready callback.");
  return true;
}

void *V5FileSystem::open_cb(const char *path, lv_fs_mode_t mode) {
  const char *flags = "";
...
lv_fs_res_t V5FileSystem::close_cb(void *file_p) {
  return fclose((FILE *)file_p) == 0 ? LV_FS_RES_OK : LV_FS_RES_UNKNOWN;
}

lv_fs_res_t V5FileSystem::read_cb(void *file_p, void *buf, uint32_t btr,
                                  uint32_t *br) {
  *br = fread(buf, 1, btr, (FILE *)file_p);
  return (int32_t)(*br) < 0 ? LV_FS_RES_UNKNOWN : LV_FS_RES_OK;
}

lv_fs_res_t V5FileSystem::write_cb(void *file_p, const void *buf, uint32_t btw,
                                   uint32_t *bw) {
  *bw = fwrite(buf, 1, btw, (FILE *)file_p);
  return (int32_t)(*bw) < 0 ? LV_FS_RES_UNKNOWN : LV_FS_RES_OK;
}

lv_fs_res_t V5FileSystem::seek_cb(void *file_p, uint32_t pos,
...
lv_fs_res_t V5FileSystem::tell_cb(void *file_p, uint32_t *pos_p) {
  *pos_p = ftell((FILE *)file_p);
  return LV_FS_RES_OK;
}

void *V5FileSystem::dir_open_cb(const char *path) {
  return nullptr;
}

lv_fs_res_t V5FileSystem::dir_read_cb(void *rddir_p, char *fn) {
    return LV_FS_RES_NOT_IMP;
}

lv_fs_res_t V5FileSystem::dir_close_cb(void *rddir_p) {
    return LV_FS_RES_NOT_IMP;
}
